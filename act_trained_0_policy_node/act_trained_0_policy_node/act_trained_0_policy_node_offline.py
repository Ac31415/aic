...
class act_trained_0_policy_node(Policy):
    """Policy node variant that loads an already-downloaded model folder.

    To use, set the environment variable `OFFLINE_POLICY_PATH` to the folder
    containing `config.json`, `model.safetensors` and the normalizer
    safetensors file. If the env var is not set, the default is
    the `act_short_all_aic_dataset_policy` folder next to this file.
    """
    ...
    def __init__(self, parent_node: Node):
        ...
        # -------------------------------------------------------------------------
        # 1. Configuration & Weights Loading (Offline)
        # -------------------------------------------------------------------------
        # Prefer environment-controlled path so user can point to downloaded HF snapshot
        default_local = Path(__file__).parent / "act_short_all_aic_dataset_policy"
        policy_dir = Path(os.environ.get("OFFLINE_POLICY_PATH", str(default_local)))

        if not policy_dir.exists():
            raise FileNotFoundError(
                f"Offline policy path not found: {policy_dir}. "
                "Set OFFLINE_POLICY_PATH to the local model folder."
            )
        ...
        self.get_logger().info("Normalization statistics loaded successfully (offline).")
    ...
    def insert_cable(
        ...
        **kwargs,
        ...
        self.policy.reset()
        self.get_logger().info(f"{self.__class__.__name__}.insert_cable() enter. Task: {task}")
        ...
        self.get_logger().info(f"{self.__class__.__name__}.insert_cable() exiting...")
        return True
