...
# Local loading: do not use huggingface_hub snapshot download for local checkpoints


class act_short_all_aic_gazebo_dataset_policy_node(Policy):
    def __init__(self, parent_node: Node):
        ...
        # -------------------------------------------------------------------------
        # Path to your checkpoint folder (relative to this script)
        policy_path = Path(__file__).resolve().parent / "trained model"

        # Validate existence early to provide a clear error if missing
        if not policy_path.exists():
            raise FileNotFoundError(
                f"Trained model directory not found: {policy_path}"
            )
        ...
        # The checkpoint contains the backbone weights already. Avoid any torchvision
        # download attempts during model construction in the offline container.
    ...
    def insert_cable(
        ...
        **kwargs,
        ...
        self.policy.reset()
        self.get_logger().info(f"act_short_all_aic_gazebo_dataset_policy_node.insert_cable() enter. Task: {task}")
        ...
        self.get_logger().info("act_short_all_aic_gazebo_dataset_policy_node.insert_cable() exiting...")
        return True
