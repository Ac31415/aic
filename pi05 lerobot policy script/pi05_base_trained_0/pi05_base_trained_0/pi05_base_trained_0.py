...
class pi05_base_trained_0(Policy):
    def __init__(self, parent_node: Node):
        ...
        # -------------------------------------------------------------------------
        repo_id = "caai-aic/trained_models_pi05_base_sfp_to_sfp_port_0_of_nic_card_mount_0_sc_to_sc_port_base_of_sc_port_0_4"
        ...
        # # 1. Load Config Manually (like in RunACT.py)
        ...
        print("pre load policy")

        # Load PI05 policy weights
        self.policy = PI05Policy.from_pretrained(repo_id)
        # self.policy = PI05Policy.from_pretrained(policy_path, local_files_only=True)
        ...
        # # 2. Instantiate Policy Architecture Base
        # self.policy = PI05Policy(config)

        # # 3. Stream native weights straight to GPU over PCIe! 
        # native_weights_path = policy_path / "model_native.safetensors"
        # native_state_dict = load_file(native_weights_path, device=str(self.device))

        # # Load into the model efficiently
        # self.policy.load_state_dict(native_state_dict, strict=False)

        print("load policy")
        self.policy.eval()
        print("eval policy")
        self.policy.to(self.device)
        print("load policy to dev")
        ...
        stats_path = (
            policy_path / "policy_preprocessor_step_2_normalizer_processor.safetensors"
        )
    ...
    def insert_cable(
        ...
        **kwargs,
        ...
        while time.time() - start_time < 30.0:
            ...
            print("raw_action (unnormalized): ", raw_action)
