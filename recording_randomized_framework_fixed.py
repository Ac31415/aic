...
AIC_ROOT = Path.home() / "ws_aic" / "src" / "aic"
...
AIC_SCENE_DBOX_PASSWORD = "minghan2024"
...
DATASET_TELEOP_TASK_ARGS = {
    ...
    "caai-aic/corrected_lab_collected_sfp_to_sfp_port_1_of_nic_card_mount_0": [
        ...
        "--teleop.task_plug_name=sfp_tip",
        "--teleop.task_module_name=nic_card_mount_0",
        "--teleop.task_port_name=sfp_port_1",
    ],
    "caai-aic/corrected_lab_collected_sfp_to_sfp_port_0_of_nic_card_mount_1": [
        "--teleop.task_cable_name=cable_0",
        "--teleop.task_plug_name=sfp_tip",
    ...
    "caai-aic/corrected_lab_collected_sfp_to_sfp_port_0_of_nic_card_mount_2": [
        "--teleop.task_cable_name=cable_0",
        "--teleop.task_plug_name=sfp_tip",
        "--teleop.task_module_name=nic_card_mount_2",
        "--teleop.task_port_name=sfp_port_0",
    ],
    "caai-aic/corrected_lab_collected_sfp_to_sfp_port_1_of_nic_card_mount_2": [
        "--teleop.task_cable_name=cable_0",
    ...
    "caai-aic/corrected_lab_collected_sfp_to_sfp_port_0_of_nic_card_mount_3": [
        "--teleop.task_cable_name=cable_0",
        "--teleop.task_plug_name=sfp_tip",
        "--teleop.task_module_name=nic_card_mount_3",
        "--teleop.task_port_name=sfp_port_0",
    ],
    "caai-aic/corrected_lab_collected_sfp_to_sfp_port_1_of_nic_card_mount_3": [
        "--teleop.task_cable_name=cable_0",
    ...
    "caai-aic/corrected_lab_collected_sfp_to_sfp_port_0_of_nic_card_mount_4": [
        "--teleop.task_cable_name=cable_0",
        "--teleop.task_plug_name=sfp_tip",
        "--teleop.task_module_name=nic_card_mount_4",
        "--teleop.task_port_name=sfp_port_0",
    ],
    "caai-aic/corrected_lab_collected_sfp_to_sfp_port_1_of_nic_card_mount_4": [
        "--teleop.task_cable_name=cable_0",
    ...
    "caai-aic/corrected_lab_collected_sc_to_sc_port_base_of_sc_port_0": [
        ...
        "--teleop.task_module_name=sc_port_0",
...
class RecordingTask:
    ...
    def _teleop_task_args(self) -> List[str]:
        return list(DATASET_TELEOP_TASK_ARGS.get(self.definition.repo_id, []))

    def _should_resume(self) -> bool:
        root = self.definition.dataset_root
        info_path = root / "meta" / "info.json"
        tasks_path = root / "meta" / "tasks.parquet"
        return info_path.exists() and tasks_path.exists()
    ...
    def prepare_dataset_root(self) -> None:
        ...
        if self._should_resume():
            return
        if info_path.exists() and not self._should_resume():
            print(
                "[warn] Incomplete local dataset metadata detected for "
                f"{self.definition.repo_id}; removing {root} and starting fresh."
            )
    ...
    def lerobot_command(self, num_episodes: int) -> List[str]:
        ...
        cmd.extend(self._teleop_task_args())
        if self._should_resume():
            cmd.append("--resume=true")
        return cmd

    def lerobot_command_cheatcodes(self, num_episodes: int) -> List[str]:
        cmd = [
            "pixi",
            "run",
            "lerobot-record",
...
def start_scene(launch_args: str, ws_path: Path) -> SceneProcess:
    ...
    command = (
        f"printf '%s\\n' {shlex.quote(AIC_SCENE_DBOX_PASSWORD)} | "
        "sudo -S -p '' -v && "
        "export DBX_CONTAINER_MANAGER=docker; "
        f"distrobox enter --root -r aic_eval -- {launch_cmd}"
    )
    process = _launch_terminal(
...
def wait_for_scene_ready(
    ...
    while time.time() < deadline:
        ...
        gz_cmd = (
            ...
            "sudo -S -p '' -v && "
            "export DBX_CONTAINER_MANAGER=docker; "
            "distrobox enter --root -r aic_eval -- pgrep -f gzserver"
        )
        gz_running = False
        try:
...
def choose_task(
    ...
    print("  [q] Quit")
    print("  [a] Record all datasets (cheatcodes)")
    ...
    while True:
        ...
        if choice in {"q", "quit", "exit"}:
            return None
        if choice in {"a", "all"}:
            return "ALL_CHEAT"  # sentinel for recording all with cheatcodes
        if choice.isdigit():
            index = int(choice)
...
def run_recording(
    ...
    cmd_args = (
        task.lerobot_command_cheatcodes(initial_num)
        if use_cheatcodes
        else task.lerobot_command(initial_num)
    )
    cmd = " ".join(shlex.quote(arg) for arg in cmd_args)
    
    # Tare force-torque sensor before starting cheatcode recording (in same terminal)
    if use_cheatcodes:
        print("[info] Taring force-torque sensor before recording (in same terminal)...")
        tare_cmd = "pixi run ros2 service call /aic_controller/tare_force_torque_sensor std_srvs/srv/Trigger"
        cmd = f"{tare_cmd} && {cmd}"
    ...
    if isinstance(proc, subprocess.Popen):
        if proc.poll() is None:
            if use_cheatcodes:
                # For cheatcodes, the teleop callback can get stuck even after recording ends.
                # Use a moderate timeout, then force-kill by process name (not subprocess handle,
                # which is gnome-terminal, not the actual lerobot-record inside bash).
                timeout_s = 60
                print(f"[info] Waiting for lerobot-record to finish (cheatcode mode, timeout: {timeout_s}s)...")
                try:
                    proc.wait(timeout=timeout_s)
                    print("[info] lerobot-record finished normally.")
                except subprocess.TimeoutExpired:
                    print(f"[warn] lerobot-record did not finish within {timeout_s}s, force-killing...")
                    # Use pkill to directly kill lerobot-record processes by name pattern
                    # (subprocess handle is gnome-terminal, not the actual recording process)
                    try:
                        subprocess.run(
                        ...
                        print(f"[warn] Error force-killing lerobot-record: {e}")
            else:
                # HF push/video finalize can take a while; avoid killing valid runs too early.
                timeout_s = 180 if num_episodes == 1 else 300
    ...
    if start_episode_count is not None:
        ...
        metadata_timeout_s = 60 if use_cheatcodes else 300
        updated_count = _wait_for_local_episode_update(
        ...
        if updated_count is not None:
            completed_episodes = max(0, updated_count - start_episode_count)
            if use_cheatcodes and completed_episodes < num_episodes:
                print(
                    "[warn] Cheatcode recording did not produce the expected number "
                    f"of new episodes for {task.definition.repo_id}. "
                    f"Expected {num_episodes}, got {completed_episodes}."
                )
...
def _terminate_gazebo_in_distrobox() -> None:
    cmd = (
        ...
        "sudo -S -p '' -v && "
        "export DBX_CONTAINER_MANAGER=docker; "
        "distrobox enter --root -r aic_eval -- pkill -f 'gzserver|gzclient|gazebo'"
    )
    try:
        subprocess.run(
            ["bash", "-lc", cmd],
    ...
    try:
        ...
        while True:
            ...
            if restart_event.is_set():
                ...
                try:
                    rec_args = (
                        task.lerobot_command_cheatcodes(1)
                        if use_cheatcodes
                        else task.lerobot_command(1)
                    )
                    rec_cmd = " ".join(shlex.quote(arg) for arg in rec_args)
                    # Tare force-torque sensor before restarting cheatcode recording (in same terminal)
                    if use_cheatcodes:
                        tare_cmd = "pixi run ros2 service call /aic_controller/tare_force_torque_sensor std_srvs/srv/Trigger"
                        rec_cmd = f"{tare_cmd} && {rec_cmd}"
...
def main() -> int:
    ...
    try:
        while True:
            ...
            # Special option: record all datasets using cheatcodes
            if isinstance(task, str) and task == "ALL_CHEAT":
                num_episodes_all = prompt_num_episodes()
                for t in tasks:
                    print(f"[info] Starting cheatcode recording for {t.definition.repo_id}")
                    config = scene_generator.generate_random_config()
                    scene_holder: Dict[str, object] = {}
                    scene = launch_scene_for_episode(
                        t.definition.repo_id,
                        config,
                        ws_path,
                        scene_holder,
                    )
                    ...
                    if not ready:
                        scene.stop()
                        print("[warn] Scene did not start; skipping this dataset.")
                        continue

                    start_count = episode_counts.get(t.definition.repo_id)
                ...
                print("[info] Finished cheatcode recording for all datasets.")
