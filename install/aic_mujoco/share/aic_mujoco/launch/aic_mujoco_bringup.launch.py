...
def launch_setup(context, *args, **kwargs):
    ...
    # On macOS: preload libpython so that rosidl_generator_py dylibs (transitively
    # linked via control_toolbox -> ${control_msgs_TARGETS}) can resolve Python C-API
    # symbols (e.g. _PyExc_RuntimeError) in the dyld flat namespace.
    # Scoped to mujoco_ros2_control ONLY — setting it globally (e.g. via
    # SetEnvironmentVariable) would inject libpython into xacro/Python subprocesses
    # and cause a hang.
    if platform.system() == "Darwin":
        _mujoco_preload_env = {
            "DYLD_INSERT_LIBRARIES": os.path.join(
                sys.base_prefix,
                "lib",
        ...
        _mujoco_preload_env = {
            # On Linux: preload system tinyxml2 to avoid symbol collision
            # with MuJoCo's bundled tinyxml2.
    ...
    node_mujoco_ros2_control = Node(
        ...
        additional_env=_mujoco_preload_env,
        remappings=[
