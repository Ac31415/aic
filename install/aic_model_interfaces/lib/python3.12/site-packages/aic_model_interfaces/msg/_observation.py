# generated from rosidl_generator_py/resource/_idl.py.em
# with input from aic_model_interfaces:msg/Observation.idl
# generated code does not contain a copyright notice

# This is being done at the module level and not on the instance level to avoid looking
# for the same variable multiple times on each instance. This variable is not supposed to
# change during runtime so it makes sense to only look for it once.
from os import getenv

ros_python_check_fields = getenv('ROS_PYTHON_CHECK_FIELDS', default='')


# Import statements for member types

import builtins  # noqa: E402, I100

import rosidl_parser.definition  # noqa: E402, I100


class Metaclass_Observation(type):
    """Metaclass of message 'Observation'."""

    _CREATE_ROS_MESSAGE = None
    _CONVERT_FROM_PY = None
    _CONVERT_TO_PY = None
    _DESTROY_ROS_MESSAGE = None
    _TYPE_SUPPORT = None

    __constants = {
    }

    @classmethod
    def __import_type_support__(cls):
        try:
            from rosidl_generator_py import import_type_support
            module = import_type_support('aic_model_interfaces')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'aic_model_interfaces.msg.Observation')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._CREATE_ROS_MESSAGE = module.create_ros_message_msg__msg__observation
            cls._CONVERT_FROM_PY = module.convert_from_py_msg__msg__observation
            cls._CONVERT_TO_PY = module.convert_to_py_msg__msg__observation
            cls._TYPE_SUPPORT = module.type_support_msg__msg__observation
            cls._DESTROY_ROS_MESSAGE = module.destroy_ros_message_msg__msg__observation

            from aic_control_interfaces.msg import ControllerState
            if ControllerState.__class__._TYPE_SUPPORT is None:
                ControllerState.__class__.__import_type_support__()

            from geometry_msgs.msg import WrenchStamped
            if WrenchStamped.__class__._TYPE_SUPPORT is None:
                WrenchStamped.__class__.__import_type_support__()

            from sensor_msgs.msg import CameraInfo
            if CameraInfo.__class__._TYPE_SUPPORT is None:
                CameraInfo.__class__.__import_type_support__()

            from sensor_msgs.msg import Image
            if Image.__class__._TYPE_SUPPORT is None:
                Image.__class__.__import_type_support__()

            from sensor_msgs.msg import JointState
            if JointState.__class__._TYPE_SUPPORT is None:
                JointState.__class__.__import_type_support__()

    @classmethod
    def __prepare__(cls, name, bases, **kwargs):
        # list constant names here so that they appear in the help text of
        # the message class under "Data and other attributes defined here:"
        # as well as populate each message instance
        return {
        }


class Observation(metaclass=Metaclass_Observation):
    """Message class 'Observation'."""

    __slots__ = [
        '_left_image',
        '_left_camera_info',
        '_center_image',
        '_center_camera_info',
        '_right_image',
        '_right_camera_info',
        '_wrist_wrench',
        '_joint_states',
        '_controller_state',
        '_check_fields',
    ]

    _fields_and_field_types = {
        'left_image': 'sensor_msgs/Image',
        'left_camera_info': 'sensor_msgs/CameraInfo',
        'center_image': 'sensor_msgs/Image',
        'center_camera_info': 'sensor_msgs/CameraInfo',
        'right_image': 'sensor_msgs/Image',
        'right_camera_info': 'sensor_msgs/CameraInfo',
        'wrist_wrench': 'geometry_msgs/WrenchStamped',
        'joint_states': 'sensor_msgs/JointState',
        'controller_state': 'aic_control_interfaces/ControllerState',
    }

    # This attribute is used to store an rosidl_parser.definition variable
    # related to the data type of each of the components the message.
    SLOT_TYPES = (
        rosidl_parser.definition.NamespacedType(['sensor_msgs', 'msg'], 'Image'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['sensor_msgs', 'msg'], 'CameraInfo'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['sensor_msgs', 'msg'], 'Image'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['sensor_msgs', 'msg'], 'CameraInfo'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['sensor_msgs', 'msg'], 'Image'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['sensor_msgs', 'msg'], 'CameraInfo'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['geometry_msgs', 'msg'], 'WrenchStamped'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['sensor_msgs', 'msg'], 'JointState'),  # noqa: E501
        rosidl_parser.definition.NamespacedType(['aic_control_interfaces', 'msg'], 'ControllerState'),  # noqa: E501
    )

    def __init__(self, **kwargs):
        if 'check_fields' in kwargs:
            self._check_fields = kwargs['check_fields']
        else:
            self._check_fields = ros_python_check_fields == '1'
        if self._check_fields:
            assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
                'Invalid arguments passed to constructor: %s' % \
                ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        from sensor_msgs.msg import Image
        self.left_image = kwargs.get('left_image', Image())
        from sensor_msgs.msg import CameraInfo
        self.left_camera_info = kwargs.get('left_camera_info', CameraInfo())
        from sensor_msgs.msg import Image
        self.center_image = kwargs.get('center_image', Image())
        from sensor_msgs.msg import CameraInfo
        self.center_camera_info = kwargs.get('center_camera_info', CameraInfo())
        from sensor_msgs.msg import Image
        self.right_image = kwargs.get('right_image', Image())
        from sensor_msgs.msg import CameraInfo
        self.right_camera_info = kwargs.get('right_camera_info', CameraInfo())
        from geometry_msgs.msg import WrenchStamped
        self.wrist_wrench = kwargs.get('wrist_wrench', WrenchStamped())
        from sensor_msgs.msg import JointState
        self.joint_states = kwargs.get('joint_states', JointState())
        from aic_control_interfaces.msg import ControllerState
        self.controller_state = kwargs.get('controller_state', ControllerState())

    def __repr__(self):
        typename = self.__class__.__module__.split('.')
        typename.pop()
        typename.append(self.__class__.__name__)
        args = []
        for s, t in zip(self.get_fields_and_field_types().keys(), self.SLOT_TYPES):
            field = getattr(self, s)
            fieldstr = repr(field)
            # We use Python array type for fields that can be directly stored
            # in them, and "normal" sequences for everything else.  If it is
            # a type that we store in an array, strip off the 'array' portion.
            if (
                isinstance(t, rosidl_parser.definition.AbstractSequence) and
                isinstance(t.value_type, rosidl_parser.definition.BasicType) and
                t.value_type.typename in ['float', 'double', 'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'int64', 'uint64']
            ):
                if len(field) == 0:
                    fieldstr = '[]'
                else:
                    if self._check_fields:
                        assert fieldstr.startswith('array(')
                    prefix = "array('X', "
                    suffix = ')'
                    fieldstr = fieldstr[len(prefix):-len(suffix)]
            args.append(s + '=' + fieldstr)
        return '%s(%s)' % ('.'.join(typename), ', '.join(args))

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.left_image != other.left_image:
            return False
        if self.left_camera_info != other.left_camera_info:
            return False
        if self.center_image != other.center_image:
            return False
        if self.center_camera_info != other.center_camera_info:
            return False
        if self.right_image != other.right_image:
            return False
        if self.right_camera_info != other.right_camera_info:
            return False
        if self.wrist_wrench != other.wrist_wrench:
            return False
        if self.joint_states != other.joint_states:
            return False
        if self.controller_state != other.controller_state:
            return False
        return True

    @classmethod
    def get_fields_and_field_types(cls):
        from copy import copy
        return copy(cls._fields_and_field_types)

    @builtins.property
    def left_image(self):
        """Message field 'left_image'."""
        return self._left_image

    @left_image.setter
    def left_image(self, value):
        if self._check_fields:
            from sensor_msgs.msg import Image
            assert \
                isinstance(value, Image), \
                "The 'left_image' field must be a sub message of type 'Image'"
        self._left_image = value

    @builtins.property
    def left_camera_info(self):
        """Message field 'left_camera_info'."""
        return self._left_camera_info

    @left_camera_info.setter
    def left_camera_info(self, value):
        if self._check_fields:
            from sensor_msgs.msg import CameraInfo
            assert \
                isinstance(value, CameraInfo), \
                "The 'left_camera_info' field must be a sub message of type 'CameraInfo'"
        self._left_camera_info = value

    @builtins.property
    def center_image(self):
        """Message field 'center_image'."""
        return self._center_image

    @center_image.setter
    def center_image(self, value):
        if self._check_fields:
            from sensor_msgs.msg import Image
            assert \
                isinstance(value, Image), \
                "The 'center_image' field must be a sub message of type 'Image'"
        self._center_image = value

    @builtins.property
    def center_camera_info(self):
        """Message field 'center_camera_info'."""
        return self._center_camera_info

    @center_camera_info.setter
    def center_camera_info(self, value):
        if self._check_fields:
            from sensor_msgs.msg import CameraInfo
            assert \
                isinstance(value, CameraInfo), \
                "The 'center_camera_info' field must be a sub message of type 'CameraInfo'"
        self._center_camera_info = value

    @builtins.property
    def right_image(self):
        """Message field 'right_image'."""
        return self._right_image

    @right_image.setter
    def right_image(self, value):
        if self._check_fields:
            from sensor_msgs.msg import Image
            assert \
                isinstance(value, Image), \
                "The 'right_image' field must be a sub message of type 'Image'"
        self._right_image = value

    @builtins.property
    def right_camera_info(self):
        """Message field 'right_camera_info'."""
        return self._right_camera_info

    @right_camera_info.setter
    def right_camera_info(self, value):
        if self._check_fields:
            from sensor_msgs.msg import CameraInfo
            assert \
                isinstance(value, CameraInfo), \
                "The 'right_camera_info' field must be a sub message of type 'CameraInfo'"
        self._right_camera_info = value

    @builtins.property
    def wrist_wrench(self):
        """Message field 'wrist_wrench'."""
        return self._wrist_wrench

    @wrist_wrench.setter
    def wrist_wrench(self, value):
        if self._check_fields:
            from geometry_msgs.msg import WrenchStamped
            assert \
                isinstance(value, WrenchStamped), \
                "The 'wrist_wrench' field must be a sub message of type 'WrenchStamped'"
        self._wrist_wrench = value

    @builtins.property
    def joint_states(self):
        """Message field 'joint_states'."""
        return self._joint_states

    @joint_states.setter
    def joint_states(self, value):
        if self._check_fields:
            from sensor_msgs.msg import JointState
            assert \
                isinstance(value, JointState), \
                "The 'joint_states' field must be a sub message of type 'JointState'"
        self._joint_states = value

    @builtins.property
    def controller_state(self):
        """Message field 'controller_state'."""
        return self._controller_state

    @controller_state.setter
    def controller_state(self, value):
        if self._check_fields:
            from aic_control_interfaces.msg import ControllerState
            assert \
                isinstance(value, ControllerState), \
                "The 'controller_state' field must be a sub message of type 'ControllerState'"
        self._controller_state = value
